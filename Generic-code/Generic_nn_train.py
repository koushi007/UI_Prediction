...
# load the dataset
path = 'Generic_data.csv'
df = read_csv(path, header=None)
# split into input and output columns
X, y = df.values[1:,1:-number_outputstates] , df.values[1:, -number_outputstates:]
...
model.fit(X_train, y_train, epochs=1800, batch_size=100,validation_split = 0.1,verbose=None,callbacks=[es])
# evaluate the model
...
model.save('Generic_{}'.format(acc))